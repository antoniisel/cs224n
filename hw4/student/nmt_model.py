...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(input_size=embed_size,
                               hidden_size=hidden_size,num_layers=1, bidirectional=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        h2 = torch.concat(tuple(h), dim=1)
        init_decoder_hidden = self.h_projection(h2)

        c2 = torch.concat(tuple(c), dim=1)
        init_decoder_cell = self.h_projection(c2)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        o_prev = torch.zeros(batch_size, self.hidden_size, device=self.DEVICE) ## python sanity_check.py 1e
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        U_t = torch.concatenate((dec_hidden,a_t), dim=-1)
